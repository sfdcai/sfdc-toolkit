...
.DESCRIPTION
    ...
    It provides a powerful menu-driven interface to manage multiple, folder-based project
...
.NOTES
    ...
    Version:    13.1.0 (Final Network Release)
    Created:    2025-07-14
    License:    MIT
...
function Show-SystemInfo {
    function Get-OrgDisplayInfo {
        param([string]$alias)
        if (-not $alias) { return @{ Display = "Not Set"; Color = "Yellow"; Api = "N/A" } }
        if ($Script:Settings.Orgs) {
            $org = $Script:Settings.Orgs | Where-Object { $_.alias -eq $alias }
            if ($org) { 
                return @{ Display = "$($org.alias) ($($org.username))"; Color = "Green"; Api = $org.instanceApiVersion }
            }
        }
        return @{ Display = "$alias (details not cached)"; Color = "Yellow"; Api = "Unknown" }
    }
...
function Show-MainMenu {
    ...
    Write-Host "  [3] Deploy Metadata (Advanced)"
    Write-Host ""
    Write-Host "  Org & Project Setup" -ForegroundColor White
    Write-Host "  --------------------------"
    Write-Host "  [4] List & Select Source/Destination Orgs"
    Write-Host "  [5] Generate Deployment Manifest (package.xml)" -ForegroundColor "Cyan"
    Write-Host "  [6] Edit Project Settings"
    Write-Host "  [7] Authorize a New Org"
    Write-Host ""
    Write-Host "  System & Advanced Tools" -ForegroundColor White
    Write-Host "  --------------------------"
    Write-Host "  [8] Open Org in Browser"
    Write-Host "  [9] Update Metadata Mappings from Org" -ForegroundColor "Magenta"
    Write-Host " [10] Analyze Local Profile/Permission Set Files"
    Write-Host " [11] View Project Log File"
    Write-Host " [12] Clear Project Cache"
    Write-Host " [13] Re-run System Readiness Check"
    Write-Host ""
...
function Check-Prerequisites {
    ...
    try {
        if (-not $ForceRefresh -and $Script:Settings.SystemInfo -and ( (New-TimeSpan -Start ([datetime]$Script:Settings.SystemInfo.LastCheck) -End (Get-Date)).TotalHours -lt 24) ) {
            ...
            $Script:Settings.SystemInfo.Software | ForEach-Object { 
                Write-Host ("  [*] $($_.Name)... ") -NoNewline
                Write-Host "[INSTALLED]" -ForegroundColor $(if($_.Installed){"Green"}else{"Red"}) 
            }
        ...
        $Script:Settings.SystemInfo = @{ LastCheck = (Get-Date -Format "o"); Software = $liveSoftwareStatus }
        Save-Settings
    } catch {
...
function Authorize-Org {
    ...
    $isProd = Read-Host "`Is this a Production/Developer Edition org? (y/n)"
...
function Handle-Comparison-SubMenu {
    ...
    Write-Log -Level DEBUG "Entering 'Compare Orgs & Generate Delta' menu."
    ...
    while ($true) {
        ...
        if ($choice -ne 'q') { Read-Host "`nPress Enter to return to the comparison menu..." }
    }
}
...
function Create-Delta-Package {
    ...
    Get-ChildItem -Path $SourcePath -Recurse -File | ForEach-Object {
        ...
        if ($isNew -or $isModified) {
            $componentAdded = Add-Component-To-Types -TypesHashtable $additiveTypes -BasePath $SourcePath -FullPath $sourceFile.FullName -UnrecognizedFolders $unrecognizedFolders
            if($componentAdded) {
                # Copy the file to the deployment package
    ...
    Get-ChildItem -Path $TargetPath -Recurse -File | ForEach-Object {
        ...
        if (-not (Test-Path $sourceFile)) {
            Add-Component-To-Types -TypesHashtable $destructiveTypes -BasePath $TargetPath -FullPath $targetFile.FullName -UnrecognizedFolders $unrecognizedFolders | Out-Null
        }
    }
...
function Add-Component-To-Types {
    ...
    try {
        $RelativePath = $FullPath.Substring($BasePath.Length).TrimStart('\/')
        $relativeParts = $RelativePath.Split([System.IO.Path]::DirectorySeparatorChar) | Where-Object { $_.Length -gt 0 }
        
        $pathPartsToAnalyze = $relativeParts
        if ($relativeParts.Count -gt 1 -and $relativeParts[0] -eq 'unpackaged') {
            $pathPartsToAnalyze = $relativeParts[1..($relativeParts.Count - 1)]
        }

        if ($pathPartsToAnalyze.Count -ge 2) { 
            $metadataTypeFolder = $pathPartsToAnalyze[0]
            ...
            $fileName = $pathPartsToAnalyze[-1]
            $componentNamePart = [System.IO.Path]::GetFileNameWithoutExtension($fileName).Split('.')[0]
            $memberName = $componentNamePart
            
            if ($metadataTypeName -in @("LightningComponentBundle", "AuraDefinitionBundle", "ExperienceBundle", "StaticResource", "ContentAsset")) {
                $memberName = $pathPartsToAnalyze[1]
            }
            
            if ($metadataTypeName -in @("Layout", "RecordType", "ValidationRule", "WebLink", "SharingReason", "ListView", "FieldSet", "CompactLayout") -and $pathPartsToAnalyze.Count -ge 3) {
                $objectName = $pathPartsToAnalyze[1].Split('.')[0]
...
function Main {
    ...
    try {
        ...
        if (-not $Script:Settings.SourceOrgAlias) {
            ...
            Check-Prerequisites
            Select-Org
            # After selecting orgs, check if the map is stale
             if ((Test-Path $Script:METADATA_MAP_FILE) -and (Get-Content $Script:METADATA_MAP_FILE | ConvertFrom-Json).lastUpdated) {
                 if (((Get-Date) - [datetime](Get-Content $Script:METADATA_MAP_FILE | ConvertFrom-Json).lastUpdated).TotalDays -gt 7) {
                    Write-Host "`nYour metadata map is older than 7 days." -ForegroundColor Yellow
                     if((Read-Host "It is recommended to update it now. Update now? (y/n)").ToLower() -eq 'y') {
                        Update-Metadata-Mappings
                     }
                 }
             } else { # If file doesn't exist at all or has no timestamp
                Write-Host "`nYour metadata map is missing or out of date." -ForegroundColor Yellow
        ...
        while ($true) {
            ...
            switch ($choice) {
                ...
                '2'  { Run-Quick-Visual-Compare; Read-Host "`nPress Enter to return to the menu..." }
                '3'  { Deploy-Metadata-Advanced } 
                '4'  { Select-Org } 
                '5'  { Handle-Manifest-Generation }
                '6'  { Edit-ProjectSettings; Read-Host "`nPress Enter to return to the menu..." } 
                '7'  { Authorize-Org } 
                '8'  { Open-Org }
                '9'  { Update-Metadata-Mappings; Read-Host "`nPress Enter to return to the menu..." }
                '10' { Analyze-Permissions-Local; Read-Host "`nPress Enter to return to the menu..." }
                '11' { View-LogFile } 
                '12' { Clear-ProjectCache; Read-Host "`nPress Enter to return to the menu..." } 
                '13' { Check-Prerequisites -ForceRefresh; Read-Host "`nPress Enter to return to the menu..." }
                's'  { Main; return }
